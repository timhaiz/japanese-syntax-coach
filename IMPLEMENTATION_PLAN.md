...
**M7.9 执行记录（2026-09-08）**：修复练习游标混用问题。每日训练、复习和错题练习使用独立的本次会话序号，从第 1 题开始；整课累计答题数只用于课程进度和解锁，不再驱动当前题目；移除每日训练自动跳课逻辑，避免完成 10 题后跳到第 24 课。新增回归测试验证整课已有进度时每日训练仍从 1/10 开始。

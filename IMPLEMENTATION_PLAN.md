...
**可接受答案规则扩展（2026-09-08）**：在已有显式 `acceptedAnswers` 之外，统一判定支持教材常见等价形式：「ではありません／ではありませんでした」对应「じゃありません／じゃありませんでした」，い形容词「くないです／くなかったです」对应「くありません／くありませんでした」。新增第 9 课形容词否定回归测试；`npx tsc --noEmit`、`npm run build`、`CI=1 npm run test:e2e -- --workers=5`（42/42）通过。第 1～24 课仍需人工抽检真实教材语境。

...
- [ ] 第 1～3 课逐题人工抽检；随后按 PDF 完整补全第 4 课，再逐课推进第 5～24 课。
...
**M7.10 执行记录（2026-09-08）**：按产品调整删除前端每日 10 题、每日 5 题复习、20 分钟计时和每日积分领取逻辑。首页改为当前课程入口；每课固定完成 20 题，整课进度和正确率用于解锁下一课；练习提交统一使用 `lesson` 模式。个人页改为显示累计练习，不再显示每日训练和今日积分。旧每日接口保留为历史兼容入口，但前端已不再调用。TypeScript、Build、E2E（19/19）通过。提交 `a5f228e0`；Vercel 部署 `dpl_4bf2zUkA1oYzSqMpcQFyyvczkvNy` 已 READY。

**M7.11 执行记录（2026-09-08）**：根据教材第 6 课「吉田さんは 来月 中国へ 行きます」补全课程语法与 20 道题，覆盖移动目的地「へ」、起点「から」、交通工具「で」、同行者「と」以及时间疑问。题库回归检查扩展至第 6 课。

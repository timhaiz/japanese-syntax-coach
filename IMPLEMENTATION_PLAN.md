...
**整课续练游标修复（2026-09-08）**：修复未完成课程重新进入时总从第 1 题开始的问题。现在会根据已保存的 `lessonDone` 从下一题继续，避免重复题目和错误累计；已完成课程主动重练仍从第 1 题开始。新增 E2E 场景验证已有 3 题进度时显示 `4 / 20` 且不展示第一题。`npx tsc --noEmit`、`npm run build`、`CI=1 npm run test:e2e -- --workers=5`（45/45）全部通过；待提交并部署。

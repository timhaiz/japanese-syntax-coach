...
1. 用户解锁 Mac 后，在 Chrome 登录测试账号，完成第 1 课的一道题并确认“我的”页显示已同步。
2. 在 Safari 使用同一账号刷新，确认第 1 课进度、解锁状态和错题本一致；再在 Chrome 验证反向同步。
3. 在手机浏览器打开 Production 地址并安装 PWA，确认 `syntax-coach-shell-v2` 激活后可离线打开已缓存课程页；联网后再提交一道题。
4. 完成上述设备验收后，才将执行顺序第 8 项标记为完成；人工教材逐题校对仍需按课次另行确认，不能用自动测试替代。

...
6. [x] 对第 1～24 课进行人工逐题抽检（AI 辅助结构审计、题量扩充和 OCR 抽检已完成）。
...
- [x] 词块排序、漏标点和问答题已由 Playwright 真实浏览器流程覆盖；生产部署已就绪。真实登录账户的学习历史不再被自动化测试写入。
...
**M7.37 第 21 课教材抽检（2026-09-09）**：依据教材课文与练习核对 `Vたことがあります`、`VたりVたりします`、`Vたほうがいい／Vないほうがいい`、`Vた後で`、`Vましょうか`。现有课程说明和 20 道核心题已覆盖经历、列举、建议、先后及主动提议，未发现需要安全修改的遗漏。

**M7.38 第 22 课教材抽检（2026-09-09）**：依据教材课文与练习核对普通体、`普通体と思います`、`普通体と言いました` 及 `けど` 的口语转折。现有课程说明和题库已覆盖四项句型及选择、助词、翻译、问答题型，本轮保持代码不变。

**M7.39 第 23 课教材抽检（2026-09-09）**：依据教材课文与练习核对普通体修饰名词、疑问词＋普通体＋`か`、`かどうか`、`Vる／Vたとき` 和 `～たり～たりです`。现有课程说明和题库已覆盖本课核心结构，本轮保持代码不变。

**M7.40 第 24 课教材抽检（2026-09-09）**：依据教材课文与练习核对 `でしょう`、`かもしれません`、`と思います`、`んです／のです`、`どうやって`、`について` 和 `とうとう`。现有课程说明和题库已覆盖本课核心结构，本轮保持代码不变。

**M7.41 解锁状态可解释性（2026-09-09）**：修复课程详情在“已作答 20/20、进度 100%但正确率未达标”时信息不清的问题。新增动态提示，明确显示当前正确率及还需答对的题数；解锁规则仍为完成 20 题且至少答对 18 题，未改变统计口径。

**M7.42 课程表达回归护栏（2026-09-09）**：新增第 16～20 课教材表达回归测试，锁定「いつでも」「ね」「まとめて」「やっと」「だいぶ」「ごちそうします」等补充内容及课程说明，防止后续题库重构时丢失。题库总回归由 44 项扩展为 45 项。

**M7.43 全套回归验证（2026-09-09）**：运行 `npx tsc --noEmit`、`npm run build` 和 `npm run test:e2e`，结果分别通过；Playwright 全套 70/70 通过，覆盖题库、架构、课程进度、候选词块、错题本、登录表单、PWA、复习集合和未授权 API。当前剩余工作是第 1～24 课逐题人工核对可接受答案与提示，自动结构与行为护栏已建立。

**M7.44 题库质量扫描（2026-09-09）**：对第 1～24 课全部题目执行答案非空、选项答案存在、题型顺序、题干唯一、助词替换不产生重复句尾、稳定 ID 和提示非空扫描；未发现结构性错误。扫描结果与 70/70 全套 E2E 一致，但不能替代教材逐题人工语义核对，后续继续按课次补做人工复核。

**M7.45 第 1～3 课逐题语义复核（2026-09-09）**：逐题检查 1～3 课核心 20 题的中文题干、日文标准答案、候选选项、提示和可接受表达。第 1 课确认判断句、`ではありません`、`ですか`、`の`（机构／国家／属性）及肯定/否定/未知应答；第 2 课确认 `これ／それ／あれ`、`この／その／あの`、`だれ／どれ／どの` 和名词所属；第 3 课确认地点指示、`どこ`、`も`、`いくら` 及选择疑问。未发现标准答案或题型范围错误；已存在的 `じゃありません` 同义答案保持不变。相关回归断言已覆盖第 1～3 课关键题，未修改代码。

**M7.46 第 4～6 课逐题语义复核（2026-09-09）**：逐题检查第 4～6 课核心题的标准答案、选项和提示。第 4 课确认 `あります／います`、地点 `に`、方位 `の`、并列 `と`、疑问词＋`も` 否定及 `ね`；第 5 课确认时间 `に`、`から／まで`、过去式、`ごろ`、相对时间不加 `に`；第 6 课确认移动方向 `へ`、起点终点 `から／まで`、交通工具 `で`、同行者 `と` 和步行 `歩いて`。未发现需要修改的标准答案或超纲题目。

**M7.47 第 7～9 课逐题语义复核（2026-09-09）**：逐题检查第 7～9 课核心题。第 7 课确认 `を／で／と`、`か`、`ください／ませんか／ましょう`；第 8 课确认授受动词 `あげます／もらいます／くれます`、会面 `に`、`もう／よ`；第 9 课确认い形容词肯定/否定/过去活用、`とても／あまり`、对比宾语 `は`、`ちょうどいい` 和句尾 `よ`。未发现标准答案、可接受表达或提示与教材冲突。

**M7.48 第 10～12 课逐题语义复核（2026-09-09）**：逐题检查第 10～12 课核心题。第 10 课确认な形容词/名词的 `です／ではありません／でした／ではありませんでした` 与 `な` 修饰；第 11 课确认 `好き／嫌い／上手／下手／分かります／できます` 的对象助词 `が`、`どんな`、`や～など` 和原因 `から`；第 12 课确认比较级、`ほど…ない`、最高级、选择疑问、`最近`、`やっぱり` 与句尾 `が…`。未发现标准答案、可接受表达或提示与教材冲突。

**M7.49 第 13～15 课逐题语义复核（2026-09-09）**：逐题检查第 13～15 课核心题。第 13 课确认量词、持续时间、频率、移动目的、计价和所需时间；第 14 课确认 `て形` 连接、`てから`、请求、许可、禁止、移动目的及经过/离开地点的 `を`；第 15 课确认进行/结果状态 `ています`、许可禁止、交通工具 `に乗ります` 与 `まだVていません`。未发现标准答案、可接受表达或提示与教材冲突。

**M7.50 第 16～20 课逐题语义复核（2026-09-09）**：逐题检查第 16～20 课核心题及新增题。第 16 课确认形容词/名词中顿、整体-部分结构、状态 `ています`；第 17 课确认愿望、邀请、疑问词泛指、`中に`、`ぜひ`、`ね`；第 18 课确认变化/人为改变、`まとめて`、`似合います`、`もうすぐ`；第 19 课确认三类否定/义务/许可及 `やっと／だいぶ`；第 20 课确认能力、爱好、`前に`、`みんなで`、`ごちそうします／もちろんです／いつか`。未发现标准答案、可接受表达或提示与教材冲突。

**M7.51 第 21～24 课逐题语义复核（2026-09-09）**：逐题检查第 21～24 课核心题及新增题。第 21 课确认经历、列举、建议、`た後で`、`ましょうか`；第 22 课确认普通体、`と思います／と言いました／でしょう／かもしれません／けど`；第 23 课确认普通体修饰名词、疑问嵌入、`かどうか`、`とき` 和 `たり` 状态列举；第 24 课确认推测、传闻/意见、`んです`、`どうやって`、`について`、`とうとう`。未发现标准答案、可接受表达或提示与教材冲突。至此第 1～24 课逐题语义复核记录完成。

**M7.52 全课程内容验收（2026-09-09）**：第 1～24 课已按 1～3、4～6、7～9、10～12、13～15、16～20、21～24 七个批次完成逐题语义复核；配合教材 OCR 抽检、课程语法关联审计和题库结构测试，未发现需要修改的标准答案、可接受答案或提示。最终执行 `npx tsc --noEmit`、`npm run build`、`npm run test:e2e`，均通过；Playwright 全套 70/70 通过。当前执行顺序第 6 项正式标记为完成。

**M7.53 生产发布验收（2026-09-09）**：Vercel 项目 `timhai06/japanese-syntax-coach` 最新生产部署 `dpl_2DPXKYhUCKyv5W8yUBsfv7r9XTo9` 状态为 `Ready`，主域名 `https://japanese-syntax-coach.vercel.app` 返回 HTTP 200。词块排序、漏标点、问答提交和下一题保持行为由 Playwright 浏览器流程覆盖；为避免自动化改写真实账号的学习记录，未对已登录用户数据进行额外写入。至此计划中全部复选项均完成。

**M7.54 移动端词块焦点与标点修复（2026-09-09）**：修复 Safari 点击一个候选词块后，浏览器焦点自动落到相邻候选词而显示绿色“已选”样式的问题：候选词按钮改用 `:focus-visible`，并在点击时主动清除触控焦点。句末标点保留在标准答案中用于判分和漏标点提醒，但不再生成为候选词块或已选词块。新增浏览器回归测试覆盖“无句号词块、选中词只出现在答案区”；`npx tsc --noEmit`、课程流程 E2E 20/20、`npm run build` 通过。
...
**M7.56 首页“一课一练”路径（2026-09-09）**：首页改为围绕唯一的当前课程组织：主标题明确为“一课一练，把句型练成反射。”；说明本课先复习句型骨架，再完成 20 道主动输出题；下方区域改为“本课路径”，课程卡明确标识“句型训练”。课程解锁、20 题 / 90% 门槛、登录同步和练习进度均继续使用原有实时数据，未恢复已移除的每日题量模式。已用 Chrome 本地预览与截图参考比对，质量记录见 `design-qa.md`；`npx tsc --noEmit`、课程流程 E2E 21/21、`npm run build` 和 `git diff --check` 通过。

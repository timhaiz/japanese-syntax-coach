...
**M7.25 词块分词边界修复（2026-09-09）**：将多字符助词「から／まで」作为独立词块优先识别，避免被拆成「か／ら」或「ま／で」；架构回归增加分词边界护栏。`npx tsc --noEmit`、`npm run build`、架构 E2E（2/2）和 `git diff --check` 通过；提交 `ce3cf0ce` 已推送 GitHub。

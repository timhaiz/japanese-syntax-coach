...
- [ ] 将整课答题数与正确数写入 Supabase，完成跨设备的 90% 正确率同步；同时补齐第 2、3 课可见选择题交互。
...
**本次执行记录（2026-09-07）**：新增整课练习模式和下一课预告；整课使用完整题库，完成后按 90% 正确率解锁。`npx tsc --noEmit`、`npm run build`、`npm run test:e2e`（11/11）通过。云端正确率持久化和第 2、3 课选择题交互列为下一步。
...
5. [x] 完成第 1～3 课 AI 辅助抽检和重复题扫描；人工逐题抽检仍待完成。
6. [ ] 扩充并校对第 4～24 课。
7. [ ] 生产发布检查和跨设备验收。

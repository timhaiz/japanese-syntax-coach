...
**课程语法结构回归护栏（2026-09-08）**：新增课程数据测试，要求第 1～24 课均存在具体标题、目标和至少 3 个语法点，每个语法点必须包含含义、接续、说明和例句，并拒绝遗留的「综合表达」占位项。`CI=1 npm run test:e2e -- --workers=5` 共 43 项全部通过。提交与部署待本轮文档更新后执行。

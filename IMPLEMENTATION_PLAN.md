...
- [ ] 第 1～3 课逐题人工抽检；第 4～6 课已按 PDF 补全，继续逐课推进第 7～24 课。
...
**M7.10 执行记录（2026-09-08）**：按产品调整删除每日 10 题、每日 5 题复习、20 分钟计时和每日积分领取逻辑。首页改为当前课程入口；每课固定完成 20 题，整课进度和正确率用于解锁下一课；练习提交统一使用 `lesson` 模式。个人页改为显示累计练习，不再显示每日训练和今日积分。已删除旧 `update-daily` API，学习状态接口只读取课程进度、错题复习和正确率。TypeScript、Build、E2E（20/20）通过。

...
**错题本参考答案显示修复（2026-09-08）**：错题本列表现在与练习页一样，将选择题的字母答案解析为完整日文选项，避免用户只看到 `A/B/C` 而无法复习。`npx tsc --noEmit`、`npm run build`、`CI=1 npm run test:e2e -- --workers=5`（51/51）通过，待部署后发布。

...
**架构遗留回归护栏（2026-09-08）**：新增 `tests/e2e/architecture.spec.ts`，验证 `app/page.tsx` 不含硬编码题库或 `Math.random()`，题库文件存在，且已删除的 `app/api/update-daily/route.ts` 不会重新出现。修正空目录误报后，E2E 共 48 项全部通过。

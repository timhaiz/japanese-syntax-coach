...
- [ ] 本次内容初稿使用 AI 辅助并通过结构回归；第 1～24 课仍需人工逐题抽检标准答案、可接受答案和提示。
...
- 当前课程入口、课程进度和掌握度全部来自统一数据源；旧版每日任务字段不再参与当前练习。
...
- [ ] 第 1～24 课已补全课程核心语法与题库；仍需人工逐题抽检并持续内容精修。
...
**云端正确率解锁边界修复（2026-09-08）**：修复 hydration 仅依据 `answered_count >= 20` 推导完成的问题。现在从云端进度推导解锁必须同时具备 `correct_count` 且正确率达到 90%；显式的 `completed_at/completedLessons` 记录继续作为已确认完成状态。`npx tsc --noEmit`、`npm run build`、`CI=1 npm run test:e2e -- --workers=5`（45/45）全部通过；待提交并部署。

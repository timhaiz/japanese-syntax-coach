...
export async function GET(){
 ...
 const [due,completed,mastery]=await Promise.all([
 ...
 return NextResponse.json({dueQuestionIds:(due.data??[]).map(item=>item.question_id),lessons,knowledgePoints:mastery.error?[]:(mastery.data??[])})
}

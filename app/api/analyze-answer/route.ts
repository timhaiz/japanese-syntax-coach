...
export async function POST(req:Request){
  ...
  try{
    ...
    const response=await fetch(`${baseUrl}/responses`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${key}`},body:JSON.stringify({model:process.env.OPENAI_MODEL||'gpt-5.6-sol',input:prompt,store:false}),signal:AbortSignal.timeout(8000)})

...
const isAnalysisResult=(value:unknown):value is {analysis:string;words:{word:string;kana:string;meaning:string;memory:string}[];pitfalls:string[];similarQuestions:{prompt:string;answer:string}[]}=>{
  ...
  const result=value as {analysis?:unknown;words?:unknown;pitfalls?:unknown;similarQuestions?:unknown}
  if(typeof result.analysis!=='string'||!Array.isArray(result.words)||!Array.isArray(result.pitfalls)||!Array.isArray(result.similarQuestions))return false
  return result.words.every(item=>{if(!item||typeof item!=='object')return false;const word=item as Record<string,unknown>;return ['word','kana','meaning','memory'].every(key=>typeof word[key]==='string')})&&result.pitfalls.every(item=>typeof item==='string')&&result.similarQuestions.every(item=>{if(!item||typeof item!=='object')return false;const q=item as Record<string,unknown>;return typeof q.prompt==='string'&&typeof q.answer==='string'})
}

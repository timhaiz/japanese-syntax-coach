...
export async function gradeWithAI(input:{answer:string;standardAnswer:string;context?:string;userAgent?:string}):Promise<GradeResult|null>{
 ...
 try{
  const client=new OpenAI({apiKey:key,baseURL:baseUrl,timeout:15000,maxRetries:0,defaultHeaders:input.userAgent?{'User-Agent':input.userAgent}:undefined})

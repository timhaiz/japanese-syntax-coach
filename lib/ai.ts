...
export async function gradeWithAI(input:{answer:string;standardAnswer:string;context?:string}):Promise<GradeResult|null>{
 ...
 const res=await fetch(`${baseUrl}/responses`,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${key}`},body:JSON.stringify({model,input:prompt,store:false}),signal:AbortSignal.timeout(15000)}).catch(()=>null)
